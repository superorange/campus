                ...
                children: list.sublist(0,index).map((f){
                  return Container(
                    ...
                    child: CachedNetworkImage(
                      imageUrl: 'http://image.hnol.net/c/2020-01/28/18/202001281805145781-1559530.jpg',
                      width: setWidth(getCount(list.sublist(0,index).length)),
                      fit: BoxFit.fitWidth,),
                  );
                }).toList()
            ),
...
squeeze: 19,
          childDelegate:ListWheelChildBuilderDelegate(builder: (BuildContext context, int index) {
            return Container(height:100,width:double.infinity,alignment: Alignment.center,child: Text(index.toString()),padding: EdgeInsets.only(bottom: 10),
